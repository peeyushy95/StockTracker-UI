...
class CandleStickChartPanToLoadMore extends React.Component {
  constructor(props) {
    ...
    const maxWindowSize = getMaxUndefined([ema26,ema45,
      ema12,
      macdCalculator,
      smaVolume50
    ]);
    ...
    const calculatedData = ema45(ema26(ema12(macdCalculator(smaVolume50(dataToCalculate)))));
  ...
  handleDownloadMore(start, end) {
    ...
    const { data: prevData, ema26, ema12,ema45, macdCalculator, smaVolume50 } = this.state;
  ...
  render() {
    ...
    const { data, ema26, ema45,ema12, macdCalculator, smaVolume50, xScale, xAccessor, displayXAccessor } = this.state;
    ...
    const showGrid = true;
    const yGrid = showGrid ? { innerTickSize: -1 * gridWidth, tickStrokeOpacity: 0.2 , tickStrokeDasharray: 'ShortDash'} : {};
    const xGrid = showGrid ? { innerTickSize: -1 * gridHeight, tickStrokeOpacity: 0.2, tickStrokeDasharray: 'ShortDash' } : {};
    ...
    return (
      <ChartCanvas ratio={ratio} width={width} height={height}
        ...
        <Chart id={1} height={400}
               yExtents={[d => [d.high, d.low], ema45.accessor(), ema26.accessor(), ema12.accessor()]}
          ...
          <CandlestickSeries />
          <LineSeries yAccessor={ema45.accessor()} stroke={ema45.stroke()}/>
          <LineSeries yAccessor={ema26.accessor()} stroke={ema26.stroke()}/>
          <LineSeries yAccessor={ema12.accessor()} stroke={ema12.stroke()}/>

          <CurrentCoordinate yAccessor={ema45.accessor()} fill={ema45.stroke()} />
